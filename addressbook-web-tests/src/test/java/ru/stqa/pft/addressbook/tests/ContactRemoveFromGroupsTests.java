...
public class ContactRemoveFromGroupsTests extends TestBase {
   ...
   public void ensurePreconditions() {
      ...
      if (maxQuantityGroups <= app.groupsCount()) {
         ...
         app.goTo().groupPage();
         while (app.db().groups().size() <= app.groupsCount()) {  // групп должно быть количество из параметров +1
            app.goTo().groupPage();
            app.group()
               .create(new GroupData().withName("TestGroupRemove " + String.format("%1$tY%1$tm%1$td%1$tH%1$tM%1$tS", LocalDateTime.now())).withHeader("TestHeader A").withFooter("TestFooter A"));
            app.goTo().gotoHomePage();
